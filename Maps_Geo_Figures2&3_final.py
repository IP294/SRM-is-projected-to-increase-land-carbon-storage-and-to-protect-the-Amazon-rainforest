...
Create figures 2&3 for SRM paper
"""
...
#%%
experiment1 = 'ssp585' #may be set as ssp245 or ssp585
experiment2 = 'G6sulfur' #SAI experiment compared to
experiments = ['ssp585', 'ssp245']

################# Specify model here ##################################
models = ['CESM2-WACCM', 'MPI-ESM1-2-LR', 'UKESM1-0-LL']

futrs1 = np.zeros((np.size(models), 181, 361))
futrs2 = np.zeros((np.size(models), 181, 361))
rels = np.zeros((np.size(models), 181, 361))
abss = np.zeros((np.size(models), 181, 361))
abss2 = np.zeros((np.size(models), 181, 361))
...
for k in range (len(varss)): #loop through variables 
    ...
    for m in range(np.size(models)): #loop through model 
        ...
        ##### select variant ID based on experiment ####
        ...
        try:
            ...
            print(model+' failed: {}'.format(e)) #exception if data fails to load 
            ...
            if var == 'NPP':
                x1 = x1* 3.154e7# *1e-12 #multiplied to make per year
                x2 = x2* 3.154e7 #*1e-12 #multiplied to make per year
            elif var == 'pr':
            ...
            ############ Specify path to land mask ######################
                ...
                land_area = np.nansum(areas[np.where((sftlf>0)&(np.isfinite(sftlf)))])
            ...
            ##calculate change between two scenarios 
    ...
    ## calculate percentage change in varianle
    if var == 'tas':
        ens_rel_mean = (ens_avr_change2/(end1_mean-273.15))*100
    else:
    ...
    ### plot maps ###   
    axes[k].text(.02, 1.07, letters[k], ha='left', va='top', transform=axes[k].transAxes, fontsize='large')
    axes[k].set_extent([lonmin ,lonmax, latmin, latmax], crs=ccrs.PlateCarree())   
    axes[k].add_feature(cfeature.COASTLINE, zorder=10)
    
    axes[k].set_xticks(np.arange(-180,210,lonstep),crs=ccrs.PlateCarree())
    axes[k].set_xticklabels(np.arange(-180,210,lonstep))
    axes[k].set_yticks(np.arange(-80,80,latstep),crs=ccrs.PlateCarree())
    axes[k].set_yticklabels(np.arange(-80,80,latstep))
    ...
    axes[k].grid(linewidth=0.5, color='black', linestyle='--')    
...
#save figure 
# fig.savefig('C:/Users/ip294/OneDrive - University of Exeter/PhD_Tipping_Points/Figures/Geoeng_paper/'+experiment1+'_vsG6sulfur_map_V2.png', bbox_inches='tight') 
